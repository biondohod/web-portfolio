...
import imagePlug from '../../resources/img/image_plug.png';
// "comments_count": 0,
// "created_at": "string",
// "description": "string",
// "id": 0,
...
const WorksItem = (props) => {
    const {user_id, name, preview, updated_at, description, likes_count, comments_count, id} = props;

    return (
        <li className="works__item">
            <div className="works-item__left-col">
                <Link to={`/${user_id}/work/${id}`}>
                    <img src={preview ? preview : imagePlug} className="works-item__img" alt="Превью сайта." width={246} height={180}/>
                </Link>
            </div>
            <div className="works-item__right-col">
                <Link to={`/${user_id}/work/${id}`}>
                    <h2 className="works-item__name">{name}</h2>
                </Link>
                <div className="works-item__info">
                    <span className="works-item__updated">{updated_at.slice(0, 10).replace('-', '.').replace('-', '.')}</span>
                ...
                <span className="works-item__descr">{description}</span>
            </div>
        </li>
    )
}
